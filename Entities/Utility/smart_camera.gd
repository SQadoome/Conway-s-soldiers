...
var old_cell: Vector2 = Vector2i.ZERO
...
func move_camera(event: InputEventMouseMotion) -> void:
	...
	var new_cell: Vector2 = TileUtil.cellurize_vector(position)
...
func scale_camera(direction: int) -> void:
	var new_zoom: Vector2 = zoom + Vector2(direction, direction)*0.05
	if not new_zoom < MIN_ZOOM and not new_zoom > MAX_ZOOM:
		zoom = new_zoom
		print(zoom)
	

func simulate_shift(by_cells: Vector2) -> void:
	assert(not by_cells == Vector2.ZERO)
	
	var new_cell: Vector2 = old_cell + by_cells
	position += TileUtil.vectorize_cell(new_cell)
	camera_shifted.emit(old_cell, new_cell)
	old_cell = new_cell
