...
func _ready() -> void:
	...
	activation_detector.set_tile(TileUtil.cellurize_vector(position) + direction)
...
func _rotate(dir: TileUtil.DIRECTIONS) -> void:
	pass
...
func _on_activation(e: Move) -> void:
	...
	for i:int in range(2, 11):
		var soldier_exists: bool = GameEvents.ingame_board_eventer.request_data(
			...
			TileUtil.cellurize_vector(position) + direction*i
	)
	...
	GameEvents.ingame_board_eventer.request_soldier_move.emit(
		TileUtil.cellurize_vector(position) + direction,
		TileUtil.cellurize_vector(position) + direction*distance
	)
