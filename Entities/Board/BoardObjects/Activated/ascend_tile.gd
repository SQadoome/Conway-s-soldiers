...
#func _enter_tree() -> void:
	#tile = TileUtil.cellurize_vector(position)
...
#func _ready() -> void:
	...
	#activation_detector.set_tile(TileUtil.cellurize_vector(global_position))
	#
	...
	#soldier_sprite.material.set_shader_parameter(
		#"influence", TileUtil.cellurize_vector(position).y/100.0
	#)
	#
#
#func _on_soldier_detected(m: Move) -> void:
		...
		#GameEvents.ingame_board_eventer.ascension.emit(
			#IngameBoardEventer.Ascension.new(
				#m.target_location,
				#TileUtil.cellurize_vector(hook_tile.global_position))
		#)
	#if count_on:
		#move_count += 1
	#
...
#func Ascend() -> void:
	...
	#var soldier_exists: bool = GameEvents.ingame_board_eventer.request_data(
		#IngameBoardEventer.DATA_REQUESTS.DOES_SOLDIER_EXIST, TileUtil.cellurize_vector(hook_tile.global_position)
	#)
	#if soldier_exists:
		#held_soldier = true
	#
...
#func UnAscend() -> void:
	...
	#
		...
		#GameEvents.ingame_board_eventer.emit_signal(
			...
			#TileUtil.cellurize_vector(hook_tile.global_position) + Vector2i.UP
		#)
		#held_soldier = false
